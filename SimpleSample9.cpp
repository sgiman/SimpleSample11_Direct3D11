...
//--------------------------------------------------------------------------------------
extern CModelViewerCamera           g_Camera;               	// камера просмотра модели
extern CDXUTDialogResourceManager   g_DialogResourceManager; 	// менеджер общих ресурсов диалогов
extern CD3DSettingsDlg              g_SettingsDlg;          	// Диалоговое окно настроек устройства
extern CDXUTTextHelper*             g_pTxtHelper;
extern CDXUTDialog                  g_HUD;						// диалог для стандартных элементов управления
extern CDXUTDialog                  g_SampleUI;					// диалоговое окно для конкретных элементов управления образца
...
//--------------------------------------------------------------------------------------
// Отклоняет любые устройства D3D9, неприемлемые для приложения, возвращая false
//--------------------------------------------------------------------------------------
bool CALLBACK IsD3D9DeviceAcceptable( D3DCAPS9* pCaps, D3DFORMAT AdapterFormat,
                                      D3DFORMAT BackBufferFormat, bool bWindowed, void* pUserContext )
{
    // Пропускать форматы бэкбуфера, которые не поддерживают альфа-смешивание
	...
	// Это приложение не определяет откат, поэтому отклоняйте любое устройство, которое
    // не поддерживает как минимум ps2.0
...
//----------------------------------------------------------------------------------------------
// Создаем любые ресурсы D3D9, которые будут работать после сброса устройства (D3DPOOL_MANAGED)
...
//----------------------------------------------------------------------------------------------
// Создаем любые ресурсы D3D9, которые не сохранятся после сброса устройства (D3DPOOL_DEFAULT)
...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        DXUT_BeginPerfEvent( DXUT_PERFEVENTCOLOR, L"HUD / Stats" ); // Эти события должны помочь PIX определить, что делает код.
        RenderText();
...
//--------------------------------------------------------------------------------------
// Освобождение ресурсов D3D9, созданных в обратном вызове OnD3D9ResetDevice.
//--------------------------------------------------------------------------------------
void CALLBACK OnD3D9LostDevice( void* pUserContext )
{
...
//--------------------------------------------------------------------------------------
// Освобождение ресурсов D3D9, созданных в обратном вызове OnD3D9CreateDevice.
//--------------------------------------------------------------------------------------
void CALLBACK OnD3D9DestroyDevice( void* pUserContext )
{
